...
{
    public partial class ExecutionLog2_Report_XtraUserControl : DevExpress.XtraEditors.XtraUserControl
    {
        public ExecutionLog2_Report_XtraUserControl()
        {
            InitializeComponent();
        }
        ...
        private readonly  ExecutionLog2AdoBLL _executionlog2AdoBll = new ExecutionLog2AdoBLL();
        ...
        private void BindData(int currentPageIndex)
        {
			...
			#region 查询条件赋值
				...
				if(ReportPath_LayoutControlItem.Visibility == LayoutVisibility.Always && ReportPath_TextEdit.Text.Trim() != string.Empty )
				{
					defaultParams.Add("ReportPath", ReportPath_TextEdit.Text.Trim());
				}
				...
				if(ReportAction_LayoutControlItem.Visibility == LayoutVisibility.Always && ReportAction_TextEdit.Text.Trim() != string.Empty )
				{
					defaultParams.Add("ReportAction", ReportAction_TextEdit.Text.Trim());
				}
            ...
            _recordCount = _executionlog2AdoBll.GetDataTableCount(defaultParams, GetSearchParams());
			
			int from = ((int)currentPageIndex - 1) * (int)_pageSize;
			int to = from+ (int)_pageSize;
			var dtSearch = _executionlog2AdoBll.GetDataTable(defaultParams, GetSearchParams(), from, to);
        ...
        private void ExecutionLog2_Report_XtraUserControl_Load(object sender, EventArgs e)
        {
            BindData(1);
