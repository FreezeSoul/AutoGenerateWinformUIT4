...
{
    public partial class ExecutionLog_Report_XtraUserControl : DevExpress.XtraEditors.XtraUserControl
    {
        public ExecutionLog_Report_XtraUserControl()
        {
            InitializeComponent();
        }
        ...
        private readonly  ExecutionLogAdoBLL _executionlogAdoBll = new ExecutionLogAdoBLL();
        ...
        private void BindData(int currentPageIndex)
        {
			...
			#region 查询条件赋值
				...
				if(ReportID_LayoutControlItem.Visibility == LayoutVisibility.Always && ReportID_TextEdit.Text.Trim() != string.Empty )
				{
					defaultParams.Add("ReportID", ReportID_TextEdit.Text.Trim());
				}
				...
				if(UserName_LayoutControlItem.Visibility == LayoutVisibility.Always && UserName_TextEdit.Text.Trim() != string.Empty )
				{
					...
					if(RequestType_LayoutControlItem.Visibility == LayoutVisibility.Always && RequestType_ComboBoxEdit.Text != "请选择" )
					{
						defaultParams.Add("RequestType",RequestType_ComboBoxEdit.Text=="是"?true:false);
					}
				...
				if(Source_LayoutControlItem.Visibility == LayoutVisibility.Always && Source_CalcEdit.Value != null && Source_CalcEdit.Text != string.Empty)
				{
					var value = new Int32();
					if(Int32.TryParse(Source_CalcEdit.Value.ToString(),out value))
					{
						defaultParams.Add("Source",value);
					}
				}
            ...
            _recordCount = _executionlogAdoBll.GetDataTableCount(defaultParams, GetSearchParams());
			
			int from = ((int)currentPageIndex - 1) * (int)_pageSize;
			int to = from+ (int)_pageSize;
			var dtSearch = _executionlogAdoBll.GetDataTable(defaultParams, GetSearchParams(), from, to);
        ...
        private void ExecutionLog_Report_XtraUserControl_Load(object sender, EventArgs e)
        {
            BindData(1);
