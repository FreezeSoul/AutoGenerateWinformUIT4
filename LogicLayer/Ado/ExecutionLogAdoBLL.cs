...
namespace BusinessLogicLayer.Ado
{
	...
	public class ExecutionLogAdoBLL
    {
        
		private ExecutionLogAdoDAL _executionlogAdoDAL = new ExecutionLogAdoDAL();
        public ExecutionLogAdoBLL()
        {
        }
        ...
        public DataTable GetDataTable(Dictionary<string,object> defaultParams, Dictionary<string, object> searchParams, int? rowStart, int? rowEnd)
        {
			...
			#region 默认查询条件
								if(defaultParams.ContainsKey("InstanceName"))
					{
						...
						paramsTemp.Add("@InstanceName",defaultParams["InstanceName"]);
            					...
            					if(defaultParams.ContainsKey("ReportID"))
					{
            								whereStr.Append(@" and ReportID=@ReportID ");
						paramsTemp.Add("@ReportID",defaultParams["ReportID"]);
										}
			...
			{
            	return _executionlogAdoDAL.GetPageDataTable(whereStr.ToString(),"InstanceName",paramsTemp);
			}
			else
			{
				return _executionlogAdoDAL.GetPageDataTable(whereStr.ToString(),"InstanceName",paramsTemp,(int)rowStart,(int)rowEnd);
			}
        }
		...
		public int GetDataTableCount(Dictionary<string,object> defaultParams, Dictionary<string, object> searchParams)
		{
			...
			#region 自定义查询
				...
				//paramsTemp.Add("columnName",searchParams["columnName"]);
			#endregion
			return _executionlogAdoDAL.GetDataTableCount(whereStr.ToString(),paramsTemp);
		}
