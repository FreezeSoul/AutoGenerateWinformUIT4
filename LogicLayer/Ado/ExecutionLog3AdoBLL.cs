...
namespace BusinessLogicLayer.Ado
{
	...
	public class ExecutionLog3AdoBLL
    {
        
		private ExecutionLog3AdoDAL _executionlog3AdoDAL = new ExecutionLog3AdoDAL();
        public ExecutionLog3AdoBLL()
        {
        }
        ...
        public DataTable GetDataTable(Dictionary<string,object> defaultParams, Dictionary<string, object> searchParams, int? rowStart, int? rowEnd)
        {
			...
			#region 默认查询条件
								if(defaultParams.ContainsKey("InstanceName"))
					{
						...
						paramsTemp.Add("@InstanceName",defaultParams["InstanceName"]);
            					...
            					if(defaultParams.ContainsKey("ItemPath"))
					{
            								whereStr.Append(@" and ItemPath=@ItemPath ");
						paramsTemp.Add("@ItemPath",defaultParams["ItemPath"]);
										}
            					...
            					if(defaultParams.ContainsKey("Parameters"))
					{
						...
						paramsTemp.Add("@Parameters",defaultParams["Parameters"]);
            					...
            					if(defaultParams.ContainsKey("ItemAction"))
					{
            								whereStr.Append(@" and ItemAction=@ItemAction ");
						paramsTemp.Add("@ItemAction",defaultParams["ItemAction"]);
										}
			...
			{
            	return _executionlog3AdoDAL.GetPageDataTable(whereStr.ToString(),"InstanceName",paramsTemp);
			}
			else
			{
				return _executionlog3AdoDAL.GetPageDataTable(whereStr.ToString(),"InstanceName",paramsTemp,(int)rowStart,(int)rowEnd);
			}
        }
		...
		public int GetDataTableCount(Dictionary<string,object> defaultParams, Dictionary<string, object> searchParams)
		{
			...
			#region 自定义查询
				...
				//paramsTemp.Add("columnName",searchParams["columnName"]);
			#endregion
			return _executionlog3AdoDAL.GetDataTableCount(whereStr.ToString(),paramsTemp);
		}
